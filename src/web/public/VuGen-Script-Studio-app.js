...
function devwebExtractorCode(corr) {
  ...
  switch (extractorType) {
    ...
    case "cookie":
      // Options form — domain and path are optional per SDK definition
    ...
    case "boundary_header":
      // Must use options form — 3-arg constructor has no 4th arg; scope must be load.ExtractorScope enum
    ...
    case "generate":
      return null; // generated at action start — not a response extractor
    case "boundary":
    default:
      // Explicit Body scope prevents false matches from response headers
...
function genMainJS(entries, correlations) {
  ...
  o +=
    '    load.log("Initializing Vuser " + load.config.user.userId, load.LogLevel.debug);\n\n';
  ...
  o +=
    '    load.log("Starting action - Iteration " + load.config.runtime.iteration, load.LogLevel.debug);\n\n';
  ...
  if (genCorrsMj.length > 0) {
    o +=
      "    // Dynamic token generators — called only for requests that need the header\n";
  ...
  // Close transaction if no markers
  if (!hasMarkers) {
    o += "    TS01.stop();\n";
    o += "    load.sleep(think_time);\n\n";
  }
  ...
  o +=
    '    load.log("Finalizing Vuser " + load.config.user.userId, load.LogLevel.debug);\n\n';
