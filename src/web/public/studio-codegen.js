...
function genCorrelationsJS(correlations) {
  ...
  // Deduplicate: same name + same config → keep first; same name + different config → rename with _2, _3
  ...
  for (const c of relevant) {
    if (!seenNames.has(c.name)) {
      ...
      // Check if it's an exact duplicate (same type + path)
      const existing = deduped.find(d => d.name === c.name);
      const sameConfig = existing &&
        existing.extractorType === c.extractorType &&
        JSON.stringify(existing.extractorConfig) === JSON.stringify(c.extractorConfig);
      if (sameConfig) continue; // true duplicate — skip silently
      // Different config with same name — rename with numeric suffix
      let suffix = 2;
      while (seenNames.has(c.name + '_' + suffix)) suffix++;
      const uniqueName = c.name + '_' + suffix;
      seenNames.add(uniqueName);
      deduped.push({ ...c, name: uniqueName });
    }
  }
...
function genDevWebUsrFile(scriptName) {
  ...
  const hasCorrJs = !!(S.scripts && S.scripts.corrjs);
  const manualExtras = [
    hasCorrJs   ? 'correlations.js='  : '',
    S.hasDpop   ? 'dpop-helper.js='   : '',
  ].filter(Boolean).join('\n');
  return `[General]
Type=DevWeb
DefaultCfg=default.cfg
...
<VugenScriptMetadata>
  ...
  <GeneralFiles>
    ...
    <FileEntry Name="ScriptUploadMetadata.xml" Filter="1" />
  ${(S.scripts && S.scripts.corrjs) ? '\t<FileEntry Name="correlations.js" Filter="2" />\n  ' : ""}${S.hasDpop ? '\t<FileEntry Name="dpop-helper.js" Filter="2" />\n  ' : ""}</GeneralFiles>
</VugenScriptMetadata>`;
}
