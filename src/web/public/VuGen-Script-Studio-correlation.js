...
function twoHarCorrelate(entries1, entries2) {
  ...
  // Mark every output correlation so the Advisor can suppress overlapping entries
  // when an array_reconstruct candidate is accepted (see advisorApplyAndRegen).
  corrs.forEach(c => { c._autoGenerated = true; });
  return corrs;
}
  ...
  for (var ci2 = 0; ci2 < candidateOrder.length; ci2++) {
    ...
    try {
      ...
      newCorrs.push({
        ...
        _autoGenerated:  true,  // eligible for suppression by Advisor array_reconstruct accept
      });
