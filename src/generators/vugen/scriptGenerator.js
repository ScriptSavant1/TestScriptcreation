...
class WebHttpScriptGenerator {
  ...
  detectNtlmKerberos() {
    ...
    for (const req of this.requests) {
      ...
      if (authType === "kerberos" || authType === "ntlm") {
        ...
        // This ensures ParameterFile.prm always uses 'username'/'password'/'domain'
  ...
  classifyVariables() {
    ...
    // JWT output vars (e.g. jwt_token set via web_js_run ResultParam) remain in
    // dynamicVarNames (Tier 1) — no ParameterFile.prm entry needed.
  ...
  generateCorrelationRegistrations(request, indent) {
    ...
    unique.forEach((corr) => {
      ...
      switch (corr.type || corr.extractorType) {
        ...
        case "id":
        case "sessionId":
          code += _vugen.emitJson(corr.name, jsonPath, indent);
          break;
        ...
        case "xpath": {
          ...
          code += _vugen.emitXpath(corr.name, xpathQuery, indent);
          break;
        }
