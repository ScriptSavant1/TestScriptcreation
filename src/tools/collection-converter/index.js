...
class BrunoDevWebConverter {
   ...
   *
   * "type": "commonjs" is explicit (not just the implicit default) because
   * main.js uses require() — Node resolves module type by walking UP the
   * directory tree for the nearest package.json, so an unrelated ancestor
   * package.json with "type":"module" (e.g. a stray file sitting in the
   * user's Downloads/extraction folder, or an IDE-scaffolded DevWeb project
   * template) would otherwise make require() throw
   * "require is not defined in ES module scope" — this file being present
   * makes this directory the authoritative one regardless of what's above it.
   */
...
- **Cause**: Node.js picks the nearest \`package.json\` by walking UP the folder tree from
  \`main.js\`. If some OTHER \`package.json\` above this folder (e.g. one already sitting in
  your Downloads folder, or one scaffolded by a DevWeb IDE project template) has
  \`"type": "module"\`, and this folder's own \`package.json\` (shipped alongside \`main.js\`)
  is missing or was not copied into the same directory, Node treats \`main.js\` as an ES
  module and \`require()\` stops being available.
- **Solution**: Make sure the \`package.json\` from this ZIP is in the SAME folder as
  \`main.js\` (it already declares \`"type": "commonjs"\` for exactly this reason — it just
  needs to actually be present there). If you're pasting these files into an existing
  DevWeb project scaffolded by an IDE, check that project's own \`package.json\` for
  \`"type": "module"\` and remove it, or copy this project's \`package.json\` over it.
