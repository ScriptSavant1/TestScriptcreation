...
> **Visual guide available:** A formatted version of this document with IIS dialog mockups is
> published alongside this file. Use it if you prefer a visual walkthrough.
...
This guide uses `C:\inetpub\lre-toolkit` as the destination. You may choose a different path — use it consistently throughout every step.
...
```cmd
C:\> mkdir C:\inetpub\lre-toolkit
```

3. Extract the deployment ZIP and copy all files from the "Files to Deploy" list into `C:\inetpub\lre-toolkit\`
4. Open File Explorer and verify: `app.js`, `web.config`, and the `src` folder must be visible **directly inside** `C:\inetpub\lre-toolkit\` — not inside a subfolder

> ⚠️ **Check the folder structure before continuing.** If everything is one level too deep (e.g., `lre-toolkit\lre-toolkit\app.js`), move all files up one level.

---

## Step 5 of 11 — Install Node.js Dependencies

This command downloads all libraries the application needs. It reads `package.json` and creates `node_modules` automatically.

```cmd
C:\> cd C:\inetpub\lre-toolkit

C:\inetpub\lre-toolkit> npm install --production
```

Wait 1–3 minutes for it to finish. At the end you will see `added NNN packages in Xs`.

> ✅ **Success:** A `node_modules` folder now appears in `C:\inetpub\lre-toolkit\`. It contains hundreds of folders — this is correct.
...
| Field | Value |
|-------|-------|
| **Name** | `lre-toolkit-pool` |
| **.NET CLR Version** | `No Managed Code` |
| **Managed Pipeline Mode** | `Integrated` |
| Start application pool immediately | ☑ checked |

5. Click **OK**
6. Find **lre-toolkit-pool** in the list → right-click → **Advanced Settings…**
...
PerfX Studio has an Admin Dashboard that shows who used which tools, error counts, file history, and more. It is protected by a password set as an IIS environment variable.

> 🔒 **Security rule:** Never put this password in `web.config`, `pm2.config.js`, or any file committed to git. Set it only inside IIS Manager as shown below. It will never appear in a URL or browser history.

**Navigation:** IIS Manager → Application Pools → lre-toolkit-pool → Advanced Settings → Environment Variables → [...]

1. In IIS Manager, click **Application Pools**
2. Right-click **lre-toolkit-pool** → click **Advanced Settings…**
3. Scroll to the **General** section → find the row **Environment Variables** → click the **[…]** button on the right
4. The **EnvironmentVariables Collection Editor** opens — click **Add**
5. Fill in the two fields:

| Field | Value |
|-------|-------|
| **Name** | `ADMIN_TOKEN` |
| **Value** | Your chosen password (at least 12 characters, mix letters and numbers) |

> ⚠️ The name must be typed exactly as `ADMIN_TOKEN` — all capitals, underscore in the middle.

6. Click **OK** to close the Collection Editor → click **OK** to close Advanced Settings
7. Right-click **lre-toolkit-pool** → click **Recycle…** to apply the change immediately

> 💡 Save this password in your team's password manager. All team members use this same password to log in to the admin dashboard. Do not share it by email.

---

## Step 8 of 11 — Create the IIS Application
...
|-------|-------|
| **Alias** | `converter` |
| **Application Pool** | click **Select…** and choose `lre-toolkit-pool` |
| **Physical Path** | `C:\inetpub\lre-toolkit` |

4. Click **OK**

> 💡 The alias `converter` must be exactly lowercase. This is why the application is accessible at `https://your-server/converter`.

---

## Step 9 of 11 — Set File Permissions

IIS runs as **NetworkService**. It needs read access to the application and write access only to the iisnode log subfolder.

```cmd
REM Read access on application folder
C:\> icacls "C:\inetpub\lre-toolkit" /grant "NetworkService:(OI)(CI)R"

REM Create log folder and grant write access to it only
C:\> mkdir "C:\inetpub\lre-toolkit\iisnode"
C:\> icacls "C:\inetpub\lre-toolkit\iisnode" /grant "NetworkService:(OI)(CI)F"
```

> ⚠️ Do not grant write access to the entire application folder — only the `iisnode\` subfolder needs it.
...
```
✅ You should see the PerfX Studio portal with tool cards.

**Test 2 — Script Studio:**
```
https://loadrunner.webdev.banksvcs.net/converter/studio
```
✅ You should see the VuGen Script Studio upload page.

**Test 3 — Admin dashboard:**
```
https://loadrunner.webdev.banksvcs.net/converter/admin
```
✅ You should see the PerfX Studio admin login page. Enter the password from Step 7.

If something does not work, see the Troubleshooting section below.
...
The password never appears in the URL or browser history. All 5 team members can log in simultaneously with the same password.

### What the dashboard shows

| Section | What it shows |
|---------|--------------|
| KPI Cards | Total conversions, success rate, errors, timeouts, average duration, unique users |
...
Use the filters at the top of the event log to narrow by:
...
| `unsupported_format` | File type not recognized | Check file matches the selected converter |
...
| `conversion_failed` | Unexpected error | Check iisnode logs in `C:\inetpub\lre-toolkit\iisnode\` |

### Downloading reports

Use the **Download** buttons at the top of the dashboard to export data as **CSV**, **Excel (XLSX)**, or **Word (DOCX)**. Reports reflect the currently selected date range and tool filter.
...
1. Extract the new deployment ZIP to a temporary folder
2. Copy all files from the "Files to Deploy" list into `C:\inetpub\lre-toolkit\`, replacing existing files
3. Do **not** delete `node_modules\` unless instructed — run npm install to update it
...
REM Recycle the app pool to load new code — no IIS restart needed
C:\inetpub\lre-toolkit> %windir%\system32\inetsrv\appcmd recycle apppool /apppool.name:"lre-toolkit-pool"
```
...
Set in IIS Manager → Application Pool → Advanced Settings → Environment Variables (Step 7). Only `ADMIN_TOKEN` is required.
...
|-------------|-------------------|-----|
| 502 Bad Gateway | Node.js failed to start | Check `C:\inetpub\lre-toolkit\iisnode\app.js-xxxxx.txt`. Also run `node app.js` from the app folder to see the error. |
| 404 Not Found | URL Rewrite not installed or web.config missing | Confirm `web.config` exists in `C:\inetpub\lre-toolkit\`. Confirm URL Rewrite installed (Step 2). Run `iisreset`. |
...
| Blank page, no error | node_modules missing | Run `npm install --production` in `C:\inetpub\lre-toolkit\`. |
| 413 Request Entity Too Large | IIS blocking large uploads | Confirm `maxAllowedContentLength="104857600"` in `web.config`. |
...
iisnode logs (check here first):
  C:\inetpub\lre-toolkit\iisnode\app.js-xxxxx.txt
