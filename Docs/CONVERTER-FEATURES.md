...
**CSVDataSet filename indirection** (`resolveCsvFilenames()`):
JMeter commonly makes CSV paths configurable via User Defined Variables:
```xml
<stringProp name="filename">${csvFile1}</stringProp>   <!-- in CSVDataSet -->
<elementProp name="csvFile1"><stringProp>users.csv</stringProp></elementProp>  <!-- in UDVs -->
```
After `convertVars()`, the filename becomes `{{csvFile1}}`. `resolveCsvFilenames()` resolves
it to `users.csv` using the UDV lookup **before** those path-variables are filtered.

**JMeter-internal variable filtering** (`filterJmxCollectionVars()`):
JMeter UDVs include execution properties that are meaningless in LoadRunner.
Three categories are removed from `collection.variable` before `classifyVariables()` runs:
- (A) Vars whose value ends in `.csv` — CSV path indirection vars (e.g. `csvFile1=users.csv`)
- (B) Known JMeter execution/scheduling names — `nrThreads`, `rampUp`, `duration`, `loopCount`, `scheduler`, etc.
- (C) Numeric-count patterns — `lines1`, `lineCount`, `rowCount`, `records1`, etc.

Without this filter, these vars enter `classifyVariables()` as Tier 2 Config params and
pollute `collection_data.csv` / `parameters.yml` with irrelevant columns.
...
**SetUp / TearDown Thread Group routing** (both single-script and multi-script mode):

| Content type in setUp/tearDown TG | DevWeb | VuGen |
|---|---|---|
| HTTP Sampler | `initialize()` / `finalize()` | `vuser_init.c` / `vuser_end.c` |
| JSR223 / BeanShell Sampler | Single `// TODO:` comment per sampler | Single `/* TODO: */` comment per sampler |

In multi-script mode, setUp/tearDown TGs still get their own directory but their
`action()` / `Action()` is empty — all content is in the lifecycle sections.

**Standalone JSR223/BeanShell samplers** (not pre/post-processors, but actual samplers in the TG):
- Stamped with `threadGroupType` by `jmxParser.js` and tracked in `standaloneScripts[]`
- Routed to the correct lifecycle section by `jmxConverter.js`
- HTTP requests in the same TG are generated as real requests in that section

The example comment layout in generated scripts:
```javascript
// DevWeb initialize() — setUp TG example
// ── SetUp Thread Group (runs once before all iterations) ──
// TODO: JSR223 setUp-sampler "Generate Keys" (groovy) — manual conversion required.
// SetUp request: POST /auth/token
const T01_response = await load.post(...)
```
```c
/* VuGen vuser_init.c — setUp TG example */
/* ── SetUp Thread Group (runs once before all iterations) ── */
/* TODO: JSR223 setUp-sampler "Generate Keys" (groovy) — manual conversion required. */
/* SetUp request: POST /auth/token */
web_custom_request("POST_auth_token", ...)
```
...
2. **Transactions List**: T01_, T02_… names, Thread Group, HTTP Method, URL
3. **LRE Setup Guide**: JMeter → LRE mapping reference table

Thread group types: Standard, SetUp, TearDown, Stepping, Ultimate, Concurrency, Arrivals.

**Implementation note**: `ws.columns` is set **without** the `header` property in all three
sheet builders. ExcelJS writes header values directly into row 1 when `header` is present,
which conflicts with the already-merged title cell in row 1 and produces a corrupt `.xlsx`.
Column headers are set manually in the dedicated header row (row 3 or 4).
